;(function (window) {
  ...
  function createQueueManager(options) {
    ...
    function applyUpdates() {
      if (!selectedId || !jsonEl) return;
      let updates = {};
      try {
        const parsed = JSON.parse(jsonEl.value || '{}');
        if (typeof parsed !== 'object' || parsed === null) throw new Error('JSON must represent an object');
        updates = parsed;
      } catch (error) {
        log(`-> JSON の解析に失敗しました: ${error.message || error}`);
        return;
      }
      fetch('/queue-update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ process_id: selectedId, updates }),
      })
        .then((res) => {
          if (!res.ok) throw new Error(res.statusText || 'Failed to update queue item');
          return res.json();
        })
        .then((data) => {
          if (data.error) throw new Error(data.error);
        ...
        .catch((error) => log(`-> キュー更新に失敗: ${error}`));
    }
